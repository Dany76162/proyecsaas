...
- 🗺️ **Ubicación geográfica / mapas**: agregado al pilar de catálogo. Real: `PropertyLocationMap` (MapLibre GL, estilo CartoDB gratuito sin API key) muestra **pin exacto** o **zona aproximada difusa** (privacidad), `enableMap` por defecto activo (`NEXT_PUBLIC_ENABLE_PROPERTY_MAP !== "false"`). Desarrollos: masterplan georreferenciado sobre satelital. ⚠️ Inconsistencia detectada: la ficha de propiedad del WORKSPACE dice "futuro mapa público" cuando en el catálogo ya se renderiza — pendiente confirmar que está activo en prod y corregir ese copy.
