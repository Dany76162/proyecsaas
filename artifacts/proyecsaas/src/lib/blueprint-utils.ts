...
export function extractPathBounds(pathData: string) {
...
export function getPathGeometry(pathData: string) {
    const coords = pathData.match(/[-+]?[0-9]*\.?[0-9]+/g)?.map(Number) ?? [];
    if (coords.length < 4) return null;
    ...
    const cx = xs.reduce((sum, val) => sum + val, 0) / xs.length;
    const cy = ys.reduce((sum, val) => sum + val, 0) / ys.length;
        ...
        if (d) {
            ...
            if (geom) {
                paths.push({
                    ...
                    areaSqm: geom.area,
                    widthDrawing: geom.width,
                    heightDrawing: geom.height
                });
            }
        }
    });
