...
export const useMasterplanStore = create<MasterplanState>((set) => ({
    // Units
    units: [],
    setUnits: (units: MasterplanUnit[]) => set({ units }),
    updateUnitState: (unitId: string, partial: Partial<MasterplanUnit>) =>
        set((state) => ({
    ...
    // Hover
    hoveredUnitId: null,
    setHoveredUnitId: (id: string | null) => set({ hoveredUnitId: id }),
    ...
    setShowComparator: (show: boolean) => set({ showComparator: show }),
