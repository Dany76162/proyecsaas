        ...
        <div className="grid gap-8 lg:grid-cols-[1.8fr_1fr]">
          ...
          <div className="space-y-6">
            <div className="sticky top-6 bg-white rounded-[2rem] border border-slate-200/80 p-6 sm:p-8 shadow-sm space-y-6">
              <div>
                <h3 className="text-lg font-extrabold text-slate-900 tracking-tight">¿Querés visitar la propiedad?</h3>
                <p className="text-xs font-semibold text-slate-500 leading-relaxed mt-1">
                  Contactanos hoy para coordinar un recorrido presencial u obtener asesoramiento inmediato de parte de nuestro equipo.
                </p>
              </div>
              ...
              <div className="space-y-3 pt-2">
                ...
                {org.contactPhone && (
                  <div className="flex w-full h-11 items-center justify-center gap-2 rounded-xl border border-slate-200 bg-slate-50 text-xs font-bold text-slate-700">
                    <Phone className="h-4 w-4 text-slate-400" />
                    Llamar al {org.contactPhone}
                  </div>
                )}

                {org.contactEmail ? (
                  <a
                    href={`mailto:${org.contactEmail}?subject=${encodeURIComponent(`Consulta: ${property.title}`)}&body=${encodeURIComponent(`Hola!\n\nEstoy interesado en la propiedad: ${property.title}.\n\nPor favor contáctenme.`)}`}
                    className="flex w-full h-11 items-center justify-center gap-2 rounded-xl border border-slate-200 hover:bg-slate-50 text-xs font-bold tracking-wider uppercase text-slate-700 transition-all duration-200"
                  >
