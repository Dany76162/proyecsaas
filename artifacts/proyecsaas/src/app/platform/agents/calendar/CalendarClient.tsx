                ...
                <div key={draft.id} className="p-4 rounded-2xl border border-slate-100 bg-white hover:border-brand-200 transition-all group">
                  ...
                  <div className="flex items-center gap-2 mt-3">
                    <Button 
                      ...
                      onClick={async () => {
                        const date = prompt("Fecha (AAAA-MM-DD):", format(new Date(), "yyyy-MM-dd"));
                        if (date) {
                          try {
                            await scheduleContentDraftAction(draft.id, { date: new Date(date), platform: draft.platform });
                            setDrafts(prev => prev.map(d => d.id === draft.id ? { ...d, calendarStatus: "SCHEDULED", scheduledFor: new Date(date) } : d));
                            alert("Programado");
                          } catch (err) {
                            alert("Error");
                          }
