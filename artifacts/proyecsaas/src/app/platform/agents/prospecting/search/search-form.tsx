...
export function ProspectingSearchForm({ serperConfigured }: { serperConfigured: boolean }) {
  ...
  // Search Fields — no hardcoded defaults so the form starts truly empty
    ...
    <div className="p-8 max-w-6xl mx-auto flex flex-col gap-8">
      ...
      {!serperConfigured && (
        <div className="rounded-2xl border border-amber-200 bg-amber-50 p-5 flex items-start gap-4">
          ...
          <div>
            <p className="text-sm font-bold text-amber-900">Buscador Web no disponible</p>
            <p className="mt-1 text-xs font-medium text-amber-800 leading-relaxed">
              Para usar el Buscador Inteligente, configurá{" "}
              <code className="font-mono bg-amber-100 px-1 rounded">SERPER_API_KEY</code>{" "}
              en las variables de entorno de Railway. Podés obtener una key gratuita en{" "}
              <span className="font-bold">serper.dev</span>.
              El tab "Pegar Datos" funciona sin esta variable.
            </p>
          </div>
        </div>
      )}
                ...
                <TabsContent value="search" className="space-y-8 mt-0 p-10">
                  {!serperConfigured ? (
                    <div className="py-12 text-center">
                      <div className="mx-auto h-16 w-16 rounded-2xl bg-amber-50 flex items-center justify-center text-amber-400 mb-4 border border-amber-100">
                        <AlertCircle className="h-8 w-8" />
                      </div>
                      <p className="text-sm font-bold text-slate-700">Buscador Web no disponible</p>
                      <p className="mt-2 text-xs font-medium text-slate-400 max-w-sm mx-auto leading-relaxed">
                        Configurá <code className="font-mono bg-slate-100 px-1 rounded">SERPER_API_KEY</code> en Railway para habilitar esta función. Mientras tanto, usá el tab "Pegar Datos".
                      </p>
                    </div>
                  ) : (
                    <>
