  ...
  return (
    <form action={action} className="flex flex-col gap-6">
        ...
        <div className="mt-5 grid gap-5">
          <div>
            ...
            <input
              ...
              placeholder="Ej: Agente Palermo, Bot de Ventas Norte"
          ...
          <div>
            ...
            <input
              ...
              placeholder="Ej: Maneja consultas de Palermo, Belgrano y Villa Crespo"
              className="mt-1.5 w-full rounded-2xl border border-slate-300 px-4 py-2.5 text-sm text-slate-950 placeholder-slate-400 focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-500/20"
            />
          </div>
          ...
          <div>
            <label className="block text-sm font-semibold text-slate-700" htmlFor="persona">
              Personalidad personalizada{" "}
              <span className="font-normal text-slate-400">(opcional)</span>
            </label>
            <p className="text-xs text-slate-400 mt-0.5">
              Instrucciones adicionales para el agente. Ej: "Mencioná siempre que somos líderes de mercado en Palermo."
            </p>
            <textarea
              id="persona"
              name="persona"
              ...
              placeholder="Instrucciones extra para el comportamiento del agente..."
              className="mt-1.5 w-full rounded-2xl border border-slate-300 px-4 py-2.5 text-sm text-slate-950 placeholder-slate-400 focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-500/20 resize-none"
            />
          </div>
        </div>
      </div>
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-2">
          ...
          >
            <input
              ...
              value="true"
              checked={is24x7}
              onChange={() => setIs24x7(true)}
              className="sr-only"
            />
            <div className="flex items-center gap-3">
              <span className="text-2xl">🌙</span>
              <div>
                <p className="font-bold text-slate-950">24/7 — Siempre disponible</p>
                <p className="text-xs text-slate-500 mt-0.5">
                  El agente responde a cualquier hora, incluso de madrugada y feriados.
                </p>
              </div>
            </div>
          </label>
          ...
          >
            <input
              ...
              value="false"
              checked={!is24x7}
              onChange={() => setIs24x7(false)}
              className="sr-only"
            />
            <div className="flex items-center gap-3">
              <span className="text-2xl">🗓</span>
              <div>
                <p className="font-bold text-slate-950">Solo en horarios configurados</p>
                <p className="text-xs text-slate-500 mt-0.5">
                  Responde dentro de los horarios de disponibilidad del módulo de Disponibilidad.
                </p>
              </div>
            </div>
          </label>
      ...
      <div className="rounded-[1.75rem] border bg-white p-6 shadow-soft">
        ...
        <p className="mt-1 text-sm text-slate-500">
          Cada agente puede manejar un número de WhatsApp Business. Los mensajes que lleguen a ese número
          serán procesados automáticamente.
        </p>
        <div className="mt-4">
          {channels.length === 0 ? (
            <div className="rounded-2xl border border-dashed border-slate-300 p-5 text-center">
              <p className="text-sm text-slate-500">
                Sin canales de WhatsApp configurados.{" "}
                <Link
            ...
            <div className="space-y-2">
              <label className="flex cursor-pointer items-center gap-3 rounded-2xl border border-slate-200 p-4 hover:border-slate-300 transition">
                <input
                  ...
                  value=""
                  defaultChecked={!agent?.whatsappChannelId}
                  className="h-4 w-4 accent-brand-500"
                />
                <div>
                  <p className="text-sm font-medium text-slate-700">Sin canal asignado</p>
                  <p className="text-xs text-slate-400">El agente no responderá mensajes</p>
                </div>
              </label>
              {channels.map((ch) => (
                <label
                  key={ch.id}
                  className="flex cursor-pointer items-center gap-3 rounded-2xl border border-slate-200 p-4 hover:border-slate-300 transition"
                >
                  <input
                    type="radio"
                    name="whatsappChannelId"
                    value={ch.id}
                    defaultChecked={agent?.whatsappChannelId === ch.id}
                    className="h-4 w-4 accent-brand-500"
                  />
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-slate-950">
                      {ch.verifiedDisplayName ?? ch.displayPhoneNumber ?? "Canal sin nombre"}
                    </p>
      ...
      <div className="rounded-[1.75rem] border bg-white p-6 shadow-soft">
        ...
        <p className="mt-1 text-sm text-slate-500">
          El agente solo recomendará propiedades del inventario que coincidan con estos filtros.
          Dejá todo vacío para que maneje todo el inventario disponible.
        </p>
        <div className="mt-5 grid gap-5">
          <div>
            <label className="block text-sm font-semibold text-slate-700">Zonas / Barrios</label>
            <p className="text-xs text-slate-400 mt-0.5">Separadas por comas. Ej: Palermo, Belgrano, Recoleta</p>
            <input
              name="zoneFilters"
              defaultValue={agent?.zoneFilters.join(", ") ?? ""}
          ...
          <div>
            ...
            {selectedTypes.map((t) => (
              <input key={t} type="hidden" name="propertyTypes" value={t} />
            ))}
          </div>
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              <label className="block text-sm font-semibold text-slate-700" htmlFor="minBudget">
                Presupuesto mínimo <span className="font-normal text-slate-400">(opcional)</span>
              </label>
              <input
                id="minBudget"
                name="minBudget"
                type="number"
                min="0"
                defaultValue={agent?.minBudget ?? ""}
                placeholder="Sin mínimo"
                className="mt-1.5 w-full rounded-2xl border border-slate-300 px-4 py-2.5 text-sm text-slate-950 placeholder-slate-400 focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-500/20"
              />
            </div>
            <div>
              <label className="block text-sm font-semibold text-slate-700" htmlFor="maxBudget">
                Presupuesto máximo <span className="font-normal text-slate-400">(opcional)</span>
              </label>
              <input
                id="maxBudget"
                name="maxBudget"
      ...
      <div className="rounded-[1.75rem] border bg-white p-6 shadow-soft">
        ...
        <h2 className="mt-2 text-lg font-semibold text-slate-950">
          ¿Cuándo pasa a un agente humano?
        </h2>
        <p className="mt-1 text-sm text-slate-500">
          Definí las reglas que determinan cuándo el agente deja de responder automáticamente y
          notifica al equipo.
        </p>
        <div className="mt-5 grid gap-5">
          <div>
            <label className="block text-sm font-semibold text-slate-700" htmlFor="escalateAfterMessages">
              Escalar después de{" "}
              <span className="font-normal text-slate-400">(mensajes sin cerrar trato)</span>
            </label>
            <div className="mt-1.5 flex items-center gap-3">
              <input
                id="escalateAfterMessages"
                name="escalateAfterMessages"
                type="number"
                min="1"
                max="20"
                defaultValue={agent?.escalateAfterMessages ?? 5}
                className="w-24 rounded-2xl border border-slate-300 px-4 py-2.5 text-sm text-slate-950 focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-500/20"
              />
              <span className="text-sm text-slate-500">intercambios de mensajes</span>
            </div>
          </div>
          <div>
            <label className="block text-sm font-semibold text-slate-700" htmlFor="escalateOnKeywords">
              Palabras clave de escalado
            </label>
            <p className="text-xs text-slate-400 mt-0.5">
              Si el cliente usa alguna de estas frases, el agente pasa a control humano inmediatamente.
              Separadas por comas.
            </p>
            <input
              id="escalateOnKeywords"
              name="escalateOnKeywords"
              defaultValue={agent?.escalateOnKeywords.join(", ") ?? ""}
              placeholder="Ej: hablar con alguien, operador, persona, gerente"
              className="mt-1.5 w-full rounded-2xl border border-slate-300 px-4 py-2.5 text-sm text-slate-950 placeholder-slate-400 focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-500/20"
            />
          </div>
          <div>
            <label className="block text-sm font-semibold text-slate-700" htmlFor="humanHandoffMessage">
              Mensaje de traspaso{" "}
              <span className="font-normal text-slate-400">(opcional)</span>
            </label>
            <p className="text-xs text-slate-400 mt-0.5">
              Lo que el agente le dice al cliente cuando escala a un humano.
            </p>
            <textarea
              ...
              defaultValue={
                ...
                "Entendido, voy a conectarte con uno de nuestros asesores. Te contactarán muy pronto."
              }
              className="mt-1.5 w-full rounded-2xl border border-slate-300 px-4 py-2.5 text-sm text-slate-950 placeholder-slate-400 focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-500/20 resize-none"
            />
          </div>
