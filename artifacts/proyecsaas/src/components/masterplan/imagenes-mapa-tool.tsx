...
export default function ImagenesMapaTool({
  proyectoId, map,
  overlayBounds = null, overlayRotation = 0, svgViewBox = null, anchorPoints = null,
  isOpen, onOpenChange,
  ...
  const loadItems = useCallback(async () => {
    setIsLoading(true);
    try {
      const res = await fetch(`/api/developments/${proyectoId}/imagenes-mapa`);
      if (res.ok) {
  ...
  // Sync URL state to restore viewerItem across reloads
  useEffect(() => {
    if (items.length > 0) {
      const urlParams = new URLSearchParams(window.location.search);
      const tourId = urlParams.get("tour");
      if (tourId && !viewerItem) {
        const item = items.find(it => it.id === tourId);
    ...
    <>
      ...
      {viewerItem && (
          <ImagenViewer
            ...
            onOpenBlueprint={() => {
              setViewerItem(null);
              window.location.href = "?tab=blueprint";
            }}
            units={units}
