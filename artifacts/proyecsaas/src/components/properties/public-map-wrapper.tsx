...
function FullCard({ prop }: { prop: any }) {
  ...
  const surface = prop.totalSurfaceM2 || prop.surfaceM2;
  return (
    <PublicPropertyCard
      imageUrl={img?.url ?? null}
      title={prop.title}
      priceLabel={fmt(prop)}
      location={[prop.neighborhood, prop.city].filter(Boolean).join(", ") || "Ubicación a confirmar"}
      badgeLabel={OP_LABEL[prop.operationType ?? ""] ?? "Propiedad"}
      organizationName={prop.organization?.name ?? "Inmobiliaria"}
      surfaceLabel={surface ? `${surface}m²` : "–"}
      bedroomsLabel={prop.bedrooms != null ? String(prop.bedrooms) : "–"}
      bathroomsLabel={prop.bathrooms != null ? String(prop.bathrooms) : "–"}
      garageLabel={prop.parkingSpots != null ? String(prop.parkingSpots) : null}
      hasTour360={(prop.panoramas || []).length > 0}
      hasPlan={Boolean(prop.floorPlanUrl)}
