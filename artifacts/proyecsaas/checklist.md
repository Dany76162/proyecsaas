...
### §5.1 WhatsApp Cloud — número demo comercial / CRM demo — 🟡 En ejecución

> Segundo número WhatsApp Cloud dedicado a demo comercial, aislado del número de soporte/productivo. Relacionado: §18 (agente demo) · §31 (demo comercial) · §41 (pre-demo).

**Estado de conexión (Meta / Cloud API):**
- ✅ Número demo conectado: **+54 9 11 6603-7971** ("Raíces Pilot Demo")
- ✅ `PHONE_NUMBER_ID`: **1138155372723730**
- ✅ Estado Meta/API: **CLOUD_API / CONNECTED**
- ✅ Envío saliente validado desde "Raíces Pilot Demo"
- 🔒 Número soporte/productivo **intacto**: **+54 9 11 6603-7990** (sin cambios)

**Fix de routing aplicado (`src/server/whatsapp/channel-resolver.ts`):**
- ✅ `resolveInboundByPhoneNumberId` consulta **DB primero** y devuelve ese canal si existe → **DB gana sobre el fallback legacy global**.
- ✅ El fallback legacy solo aplica si **no hay match en DB** Y el `phoneNumberId` entrante coincide **exactamente** con `WHATSAPP_PHONE_NUMBER_ID` global.
- ✅ **Riesgo mitigado:** un `phoneNumberId` desconocido (no en DB, ≠ global) ya **NO cae al canal global** → retorna `null` + warning `db-unresolved` (sin secretos).
- ✅ Validado: `tsc --noEmit` pasa. Webhook intacto (descarta `null` sin crashear).

**Auditoría funcional soporte/demo/comercial (2026-06-23):**
- ✅ Diseño operativo documentado en `PLAN.md`: prompts/roles separados para soporte B2B, recepción demo `[[MODO_RECEPCION]]` y comercial real/demo→cliente.
- ✅ Diagnóstico completado esta sesión: soporte/plataforma, demo comercial, transición demo→cliente, mensajes/tiempos y riesgos de mezcla.
- 🔒 Decisión operativa: NO ejecutar `--execute` hasta cerrar resolver + deploy y validar separación soporte/demo/comercial real.
- 🟠 Pendiente: separar explícitamente soporte, demo comercial y comercial/ventas real con prompts/agentes/canales propios. Riesgo confirmado: el prompt inmobiliario por defecto no sirve para soporte/demo.
- 🟠 Pendiente: plantillas Meta aprobadas para mensajes proactivos fuera de ventana 24h.
- 🟠 Pendiente: validación Inbox IA demo end-to-end (mensaje entrante → lead demo → respuesta desde número demo).

**Seed de provisión (`scripts/seed-demo-org.ts`):**
...
- 🟠 Crear/verificar organización `raicespilot-demo` (org + admin + suscripción IA + canal).
- 🟠 Conectar el número demo al **CRM demo** y validar el **Inbox IA** de la org demo (mensaje entrante → lead en org demo → respuesta desde el número demo).

**Reglas / invariantes:**
- 🚫 **Prohibido mezclar demo con soporte.** El guard de soporte filtra por `WHATSAPP_ORGANIZATION_ID`; la org demo debe ser distinta.
- 🔒 No tocar `WHATSAPP_PHONE_NUMBER_ID`, `WHATSAPP_TOKEN_ENCRYPTION_KEY` ni `WHATSAPP_WEBHOOK_VERIFY_TOKEN` globales.
...
- 🟠 Agente demo (org raicespilot-demo) pendiente de creación → ver §5.1.
- ✅ Auditoría funcional 2026-06-23: prompt/agente demo debe ser B2B comercial, no inmobiliaria vendedora; pendiente configurar/validar Inbox IA demo.
- 🟠 Diseño pendiente de implementación: prompt recepción demo con `[[MODO_RECEPCION]]`, reglas de calificación/rechazo y pausa a humano.
...
* **Arquitectura:** Cero migraciones, no se expandió `AgentType`, no se agregó columna `Agent.slug`. Identidad por `config.slug`. Fase 2A (`getAgentSpecialistReports()`) sigue funcionando igual.
...
- 🟡 Demo IA real por número demo Cloud (+5491166037971) en provisión → ver §5.1.
- ✅ Auditoría funcional 2026-06-23 completada: flujo demo→cliente queda manual/HITL hasta validar empresa, crear org real, conectar WhatsApp propio y probar First WOW.
- 🟠 Pendiente decisión comercial: mantener número demo solo para recepción o abrir tercer número comercial separado para ventas reales.
...
- 🟡 Número demo WhatsApp Cloud + CRM demo en ejecución → ver §5.1.
- ✅ Auditoría pre-demo 2026-06-23: no ejecutar `--execute` ni deploy operativo hasta cerrar separación soporte/demo/comercial, plantillas proactivas e Inbox IA demo.
- 🟠 Orden recomendado: deploy resolver+seed+docs → seed `--execute` autorizado → validar Inbox IA demo → recién después implementar prompt demo aprobado.
