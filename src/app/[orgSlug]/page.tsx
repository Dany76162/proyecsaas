...
import { AlertsCenter } from "@/components/workspace/alerts-center";
...
export default async function OrganizationHomePage({
  ...
  const subscriptionStatus = await getOrgSubscriptionStatus(organization.id);
  ...
  return (
    <>
      ...
      {!subscriptionStatus.isActive && (
        <div className="mt-4 rounded-2xl border border-amber-300 bg-amber-50 px-5 py-4 text-sm text-amber-900">
          <p className="font-semibold">Tu suscripción no está activa.</p>
          <p className="mt-1">
            El agente IA y la sincronización de propiedades están pausados.{" "}
            <Link
      ...
      {/* ── KPIs Row ── */}
      <section className="mt-5 grid grid-cols-2 gap-3 lg:grid-cols-3 lg:gap-4">
        <MetricCard
          label="Nuevos Leads"
      ...
      <section className="mt-5 grid grid-cols-1 gap-4 lg:grid-cols-3 lg:gap-5">
        ...
        <div className="space-y-5 lg:col-span-2">
              ...
              <div className="mt-5 border-t border-slate-100 pt-4">
                ...
                <div className="space-y-2">
                  {leads.slice(0, 3).map((lead) => (
                    <div
                      key={lead.id}
                      className="flex items-center justify-between rounded-xl border border-slate-100 bg-white px-3 sm:px-4 py-3 shadow-sm"
                    >
                      ...
                      <div className="ml-2 sm:ml-4 flex shrink-0 items-center gap-2 sm:gap-3">
                        <span className="hidden sm:inline text-xs font-medium text-slate-500">{lead.ownerName}</span>
                        <StatusBadge
          ...
          >
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="flex flex-col rounded-xl border border-indigo-100 bg-indigo-50 p-4 sm:p-5">
              ...
              <div className="flex flex-col rounded-xl border border-amber-100 bg-amber-50 p-4 sm:p-5">
                ...
                <p className="mt-1 text-sm text-slate-600">{unreadNotificationsCount} evento{unreadNotificationsCount !== 1 ? "s" : ""} pendiente{unreadNotificationsCount !== 1 ? "s" : ""}</p>
                <StatusBadge label={unreadNotificationsCount > 0 ? "Requiere acción" : "Sin pendientes"} tone={unreadNotificationsCount > 0 ? "warning" : "success"} />
              </div>
            </div>
          </SectionCard>
        </div>
        ...
        <div className="space-y-5">
          ...
          >
            <AlertsCenter orgSlug={orgSlug} notifications={notifications} />
          </SectionCard>
        </div>
      </section>
    </>
